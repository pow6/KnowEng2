...
#define teisuOfB 10.0             //マハラノビス距離、定数B
...
void main()
{
    ...
    FILE *fpRead;
    char resultName[40];
    sprintf(resultName,"./Result%d.txt",teisuOfB);
...
int mahalanobis(double rawData[])
{
    ...
    double sortedvector[numOfFeature][numOfFeature],average[numOfFeature];
    for(i=0;i<46;i++){
        ...
        readDataLine(average,fnaverage,numOfFeature);
...
double calcMahalanobis(double rawData[],double sortedvalue[],double sortedvector[][numOfFeature],double average[])
{
    ...
    for(k=0;k<teisuOfN;k++){
        ...
        for(i=0;i<numOfFeature;i++){
    		...
    		printf("[%d,%d]ans = %lf ch = %lf ave = %lf\n",k,i,(rawData[i]-average[i]),rawData[i],average[i]);
            //printf("[%d,%d] answer = %lf\n",k,i,(rawData[i]-average[i]));
    	//	printf("[%d,%d] fake = %lf vector = %lf\n",k,i,child,sortedvector[i][k]);
        }
    //    printf("[%d] fake = %lf\n",k,child);
        child = child * child;
        //printf("[%d] fake = %lf\n",k,child);
